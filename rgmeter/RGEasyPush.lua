...
local function easyPushButton( obj, cb, params)
	...
	local params = params or {}
	local pressedScale		= params.pressedScale
	local normalScale		= params.normalScale 
	local pressedScale		= params.pressedScale
	local normalEasing 		= params.normalEasing or params.easing 
	local pressedEasing 	= params.pressedEasing or params.easing 
	local pressedScaleTime	= params.pressedScaleTime or params.scaleTime or 100
	local normalScaleTime	= params.normalScaleTime or params.scaleTime or 100
	local cbDelay			= params.cbDelay or 150
	local normalFill		= params.normalFill 
	local pressedFill		= params.pressedFill
	local hideOnPressed 	= params.hideOnPressed or false
	local timeout 			= params.timeout or 0
	local allowPassThrough 	= params.allowPassThrough or false
